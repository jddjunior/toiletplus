...
(function () {
  ...
  function Booking({ onClose }) {
    ...
    const pad = isMobile ? '18px 18px 24px' : '20px 26px 28px';

    const shell = (children) => React.createElement('div', {
      onClick: onClose,
      style: { position: 'fixed', inset: 0, background: 'rgba(7,30,61,0.55)', backdropFilter: 'blur(4px)', zIndex: 100, display: 'flex', alignItems: isMobile ? 'flex-end' : 'center', justifyContent: 'center', padding: isMobile ? 0 : 24 },
    },
      React.createElement('div', { onClick: (e) => e.stopPropagation(), style: { background: '#fff', borderRadius: isMobile ? 'var(--radius-xl) var(--radius-xl) 0 0' : 'var(--radius-xl)', width: isMobile ? '100%' : 'min(560px, 100%)', maxHeight: isMobile ? '94vh' : '90vh', overflow: 'auto', boxShadow: 'var(--shadow-xl)' } }, children));

    const header = React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: isMobile ? '18px 18px 0' : '22px 26px 0' } },
      React.createElement('div', { style: { display: 'flex', gap: isMobile ? 10 : 18, flexWrap: 'wrap' } },
    ...
    if (step === 1) {
      return shell([
        ...
        React.createElement('div', { key: 'b', style: { padding: pad } },
          React.createElement('h3', { style: { fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: isMobile ? 22 : 26, color: 'var(--text-strong)', margin: '0 0 6px' } }, 'What\u2019s going on?'),
          ...
          React.createElement('div', { style: { display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 16, marginBottom: 22 } },
    ...
    if (step === 2) {
      return shell([
        React.createElement('div', { key: 'h' }, header),
        React.createElement('div', { key: 'b', style: { padding: pad } },
          React.createElement('h3', { style: { fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: isMobile ? 22 : 26, color: 'var(--text-strong)', margin: '0 0 6px' } }, 'Where do we text you?'),
          ...
          React.createElement('div', { style: { display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 16, marginBottom: 16 } },
