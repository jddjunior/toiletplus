...
(function () {
  ...
  const COLS = [
    ['Services', ['Drain Cleaning', 'Water Heaters', 'Leak Repair', 'Sewer & Main Line', 'Repiping']],
    ['Company', ['About Toilet Plus', 'Our Plumbers', 'Service Area', 'Careers', 'Reviews']],
    ['Resources', ['Upfront Pricing', 'Financing', 'Maintenance Tips', 'Emergency Guide', 'Contact']],
  ];

  function Footer() {
    const { Logo, Button } = window.ToiletPlusDesignSystem_5215f9;
    const { isMobile } = window.TPKit.useViewport();
    ...
    return React.createElement('footer', { style: { background: '#05152B', color: 'var(--text-on-dark-muted)' } },
      ...
      React.createElement('div', { style: { borderBottom: '1px solid var(--border-on-dark)' } },
        React.createElement('div', { style: { maxWidth: 'var(--container-max)', margin: '0 auto', padding: `${isMobile ? 28 : 36}px ${hpad}px`, display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: 20, flexWrap: 'wrap' } },
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 16 } },
            React.createElement('span', { style: { width: 46, height: 46, flex: 'none', borderRadius: 12, background: 'var(--color-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center' } }, React.createElement(Icon, { name: 'Phone', size: 22, color: '#fff' })),
            React.createElement('div', null,
              React.createElement('div', { style: { color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: isMobile ? 19 : 22 } }, 'Got a plumbing emergency right now?'),
      ...
      // columns
      React.createElement('div', { style: { maxWidth: 'var(--container-max)', margin: '0 auto', padding: `${isMobile ? 36 : 56}px ${hpad}px 36px`, display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1.4fr 1fr 1fr 1fr', gap: isMobile ? 28 : 40 } },
        React.createElement('div', { style: isMobile ? { gridColumn: '1 / -1' } : null },
        ...
        COLS.map(([title, links]) =>
          React.createElement('div', { key: title },
            ...
            React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 10 } },
              links.map((l) => React.createElement('a', { key: l, href: '#', style: { color: 'var(--text-on-dark-muted)', textDecoration: 'none', fontSize: 14 } }, l)))))),
      // legal
      React.createElement('div', { style: { borderTop: '1px solid var(--border-on-dark)' } },
        React.createElement('div', { style: { maxWidth: 'var(--container-max)', margin: '0 auto', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, fontSize: 13 } },
          ...
          React.createElement('span', { style: { display: 'flex', gap: 20 } },
            React.createElement('a', { href: '#', style: { color: 'var(--text-on-dark-muted)', textDecoration: 'none' } }, 'Privacy'),
            React.createElement('a', { href: '#', style: { color: 'var(--text-on-dark-muted)', textDecoration: 'none' } }, 'Terms'),
