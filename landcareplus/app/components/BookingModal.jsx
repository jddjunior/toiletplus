...
export function BookingModal({ onClose }) {
  ...
  const shell = (children) => (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(15,46,22,0.55)', backdropFilter: 'blur(4px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
    ...
    return shell(
      <>
        ...
        <div style={{ padding: '20px 26px 28px' }}>
          <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 26, color: 'var(--text-strong)', margin: '0 0 6px' }}>What does your yard need?</h3>
          ...
          <Select label="Type of job" required style={{ marginBottom: 16 }}>
            ...
            <option>Mulch, sod & storm cleanup</option>
            <option>Something else</option>
          </Select>
          <div style={{ display: 'flex', gap: 16, marginBottom: 22, flexWrap: 'wrap' }}>
            <Input label="ZIP code" icon={<Icon name="MapPin" size={18} />} placeholder="33602" style={{ flex: 1, minWidth: 140 }} />
            <Select label="Timing" style={{ flex: 1, minWidth: 140 }}>
              <option>This week</option>
              <option>Storm cleanup &mdash; ASAP</option>
    ...
    return shell(
      <>
        ...
        <div style={{ padding: '20px 26px 28px' }}>
          ...
          <Input label="Your name" required placeholder="Jane from South Tampa" style={{ marginBottom: 16 }} />
          <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
            <Input label="Mobile" required icon={<Icon name="Phone" size={18} />} placeholder="(813) 555-0184" style={{ flex: 1, minWidth: 160 }} />
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <Button variant="primary" size="lg" block onClick={() => setStep(3)}>Request my quote</Button>
          </div>
        </div>
      </>
    );
  ...
  return shell(
    <div style={{ padding: '40px 32px 36px', textAlign: 'center' }}>
      ...
      <p style={{ fontSize: 16, color: 'var(--text-body)', lineHeight: 1.55, margin: '0 auto 22px', maxWidth: 380 }}>
        A real crew lead is reviewing your request now. Watch for a text from <strong>(813) 555-0184</strong> with your arrival window &mdash; usually within 15 minutes.
      </p>
