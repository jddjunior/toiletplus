...
import { getServiceBySlug, getServiceSlugs } from '../../../../../lib/sanity-landcareplus';
  ...
  const graph = [
    {
      ...
      provider: { '@type': 'LandscapingBusiness', name: 'LandcarePlus' },
    },
    {
      '@type': 'BreadcrumbList',
      itemListElement: [
...
export default async function ServicePage({ params }) {
  ...
  return (
    <>
      ...
      />
      <section style={{ position: 'relative', background: 'linear-gradient(165deg, #1D5C29 0%, #0F2E16 72%)', color: '#fff', overflow: 'hidden' }}>
        ...
        <div className="tp-container" style={{ position: 'relative', padding: '48px 24px 64px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 44, letterSpacing: '-0.02em', margin: '0 0 18px', color: '#fff', maxWidth: 720 }}>{service.title}</h1>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            <ServiceCTA label={`Get a quote for ${service.title}`} />
            <Button variant="on-dark" size="lg" iconLeft={<Icon name="Phone" size={18} />} href="tel:8135550184">Call (813) 555-0184</Button>
          </div>
        </div>
      </section>
